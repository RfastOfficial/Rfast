...
double g2Statistic(vector<int>& counts, int xdim, int ydim) {
    double statistic = 0;
    if (counts.size()) {
        int countsXY = 0;
        vector<int> countsX(xdim);
        vector<int> countsY(ydim);
...
double chi2Statistic(vector<int>& counts, int xdim, int ydim) {
    double statistic = 0;
    if (counts.size()) {
        int countsXY = 0;
        vector<int> countsX(xdim);
...
static int totalCounts(vector<int>& counts, int xdim, int ydim) {
    int countsXY = 0;
    if (counts.size()) {
        for (int x = 0; x < xdim; ++x) {
            for (int y = 0; y < ydim; ++y) {
...
TestResult permG2Test(IntegerMatrix& data, int x, int y, IntegerVector& cs, IntegerVector& dc, int nperm) {
    ...
    int size = prod[ncs];
    vector<vector<int>> counts(size,vector<int>());
