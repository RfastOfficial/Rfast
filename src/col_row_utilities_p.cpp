...
IntegerMatrix col_order_p(NumericMatrix x, const bool stable, const bool descending, const unsigned int cores)
{
  ...
  imat ff(reinterpret_cast<imat::elem_type*>(f.begin()), nrw, ncl, false);
#ifdef _OPENMP
#pragma omp parallel for num_threads(cores)
#endif
  for (int i = 0; i < ncl; ++i)
  {
    ff.col(i) = Order<arma::Col<imat::elem_type>, colvec>(xx.col(i), stable, descending, 1);
  }
  return f;
}
...
#pragma omp parallel for num_threads(cores)
  ...
  for (int i = 0; i < nrw; ++i)
  {
    ff.row(i) = Order<arma::Row<imat::elem_type>, rowvec>(xx.row(i), stable, descending, 1);
  }
  return f;
}
...
SEXP col_sums_p(SEXP x, const unsigned int cores)
{
  ...
  switch (Rfast::Type::type<SEXP>(x))
  {
    ...
    default:
    {
      F = PROTECT(Rf_allocVector(INTSXP, n));
      int *FF = INTEGER(F);
      imat xx(reinterpret_cast<imat::elem_type*>(INTEGER(x)), Rf_nrows(x), n, false);
      #ifdef _OPENMP
      #pragma omp parallel for num_threads(cores)
      #endif
      for (int i = 0; i < n; i++)
...
SEXP row_sums_p(SEXP x, const unsigned int cores)
{
  ...
  switch (Rfast::Type::type<SEXP>(x))
  {
    ...
    default:
    {
      F = PROTECT(Rf_allocVector(INTSXP, n));
      int *FF = INTEGER(F);
      imat xx(reinterpret_cast<imat::elem_type*>(INTEGER(x)), n, Rf_ncols(x), false);
      #ifdef _OPENMP
      #pragma omp parallel for num_threads(cores)
      #endif
      for (int i = 0; i < n; i++)
...
SEXP col_all_p(LogicalMatrix x, const unsigned int cores)
{
  ...
  imat xx(reinterpret_cast<imat::elem_type*>(x.begin()), x.nrow(), n, false);
  int *ff = LOGICAL(f);
#ifdef _OPENMP
#pragma omp parallel for num_threads(cores)
#endif
...
SEXP row_all_p(LogicalMatrix x, const unsigned int cores)
{
  const int n = x.nrow();
  SEXP f = PROTECT(Rf_allocVector(LGLSXP, n));
  imat xx(reinterpret_cast<imat::elem_type*>(x.begin()), n, x.ncol(), false);
  int *ff = LOGICAL(f);
#ifdef _OPENMP
#pragma omp parallel for num_threads(cores)
#endif
...
IntegerVector col_count_values_p(NumericMatrix x, NumericVector values, const unsigned int cores)
{
  ...
  ivec ff(reinterpret_cast<ivec::elem_type*>(f.begin()), n, false);
