...
template<class T> struct HashBase {
    inline T operator()(T& a){ return a; }
};

template<> struct HashBase<SEXP> {
    inline intptr_t operator()(SEXP& a){ return (intptr_t)a & 0xffffffff; }
};

template<> struct HashBase<double> {
    union uno { double d; unsigned int u[2]; } tpv;
...
template<> struct HashBase<Rcomplex> {
    union uno { double d; unsigned int u[2]; } tpv;
    Rcomplex tmp;
    ...
    friend bool operator==(Rcomplex& x,Rcomplex &y){
        return (N_ISNAN(x.r, x.i) && N_ISNAN(y.r, y.i)) ? (x.r == y.r && x.i == y.i) : 
        (C_IsNA(x) ? C_IsNA(y) : (C_IsNA(y) ? 0 : (C_ISNAN(x.r, y.r) && C_ISNAN(x.i, y.i))));
    }
};

template<class T, class Hash_Helper = HashBase<T>> class Set : protected Hash<T> {
    T *data;
	size_t n, un_len = 0;
...
public:
    ...
    Group(SEXP x, SEXP ina, Function func, Hash_Helper hash_helper = Hash_Helper()) : func(func), hash_helper(hash_helper){
        this->data = Rfast::asPtr<T>(x);
